...
export class UsersRepository {
  ...
  async getUsers() {
    const users = await this.userRepository.find({
      where: { active: true },
      relations: { plots: true, supplies: true, roles: true },
    });
  ...
  async changePassword(id: UUID, changePasswordDto: ChangePasswordDto) {
    ...
    const comparation: boolean = await bcrypt.compare(
    ...
    return {
      message: 'Se ha cambiado con éxito la contraseña del usuario',
      userId: user.id,
    };
  }
  ...
  async updateUser(id: UUID, updateUserDto: UpdateUserDto) {
    if (updateUserDto.email) {
      const emailFound = await this.userRepository.findOne({
        where: { email: updateUserDto.email, active: true },
      });
  ...
  async givePremiumMonthly(userId: UUID) {
    ...
    const premiumRole = await this.roleRepository.findOne({
      where: { name: RolesEnum.PREMIUM },
    });
    const user = await this.userRepository.findOne({
  ...
  async freeTrial(userId: UUID) {
    ...
    if (user.freeTrialUsed === true) {
      throw new ConflictException('el usuario ya usó su prueba gratuita');
    }
  ...
  async giveAdmin(id: UUID) {
    ...
    if (!user) {
      throw new NotFoundException(
        `No se pudo encontrar el usuario con id:${id}`,
      );
    }
  ...
  async premiumCheck() {
    ...
    for (const user of users) {
      if (
        user.roles.some((role) => role.name === premiumRole.name) &&
        user.premiumExpiration < todayDate
      ) {
        const indexPremium = user.roles.findIndex(
          (role) => role.name === premiumRole.name,
        );
        if (indexPremium !== -1) {
          user.roles.splice(indexPremium, 1);
          this.emailService.expiredSuscription(
            user.email,
            user.name + ' ' + user.surname,
          );
        }
  ...
  async notifyUsers() {
    ...
    for (const user of users) {
      if (user.changeToday === false) {
        console.log('aca va el email sisi entendes');
      }
  ...
  async notifyIncomingExpiration() {
    ...
    for (const user of users) {
      if (user.roles.some((role) => role.name === premiumRole.name)) {
        ...
        if (comparation) {
          this.emailService.expiredSevenDays(
            user.email,
            user.name + ' ' + user.surname,
          );
        }
